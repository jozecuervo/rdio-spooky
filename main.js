...
var trackIndex = null;
var tracks = [
  { k:'t8410537', i: 75, o: 84, u: 's44885' },
  { k:'t2749998', i: 0, o: 8, u: 's400334' },
  { k:'t2767747', i: 0, o: 10, u: 's21247323' },
  { k:'t24599686', i: 48, o: 55, u: 's638438' },
  { k:'t2947818', i: 44, o: 51, u: 's1009216' },
  { k:'t2816863', i: 57, o: 65, u: 's549735' },
  { k:'t11592141', i: 0, o: 15, u: 's725446' },
  { k:'t3060435', i: 123, o: 130, u: 's662402' },
  { k:'t1235778', i: 47, o: 53, u: 's1222618' },
  { k:'t3333875', i: 38, o: 53, u: 's555990' },
  { k:'t11452757', i: 48, o: 53, u: 's1815009' },
  { k:'t2273253', i: 47, o: 62, u: 's28380175' },
  { k:'t2848933', i: 64, o: 71, u: 's352160' },
  { k:'t7234032', i: 47, o: 58, u: 's637' },
  { k:'t38896790', i: 233, o: 248, u: 's1097767' },
  { k:'t2096445', i: 23, o: 38, u: 's30183922' },
];
...
function sequenceNextTrack(index) {
  console.info('sequenceNextTrack',index);
  if (index != undefined) {
    trackIndex = index;
    // console.warn(123);
  ...
  console.info(tracks[trackIndex].k)
  R.player.play({
    source: tracks[trackIndex].k,
    initialPosition: tracks[trackIndex].i
  });
}
...
function updateUserDetails(){
  // console.info('updateUserDetails')
  $('#source .text').text('');
  $('#source .icon').attr('src', '');
  $('#upnext .text').text('');
  $('#upnext .icon').attr('src', '');
  if (tracks[trackIndex].u){
    rdioGetUser(tracks[trackIndex].u, function(user){
      // console.info('user', user)
      $('#source .text').text(user.firstName + ' ' + user.lastName);
      $('#source .icon').attr('src', user.icon);
    });
  }
  if (tracks[trackIndex+1].u){
    rdioGetUser(tracks[trackIndex+1].u, function(nextuser){
      // console.info('user', user)
      $('#upnext .text').text(nextuser.firstName + ' ' + nextuser.lastName);
      $('#upnext .icon').attr('src', nextuser.icon);
    });
  }
}
...
function rdioGetUser(key, callback) {
  R.request({
    method: 'get',
    content: {
      keys: [key]
    ...
    success: function(response) {
      console.info('user', response.result[key]);
      callback(response.result[key])
    },
    error: function(err){
...
function bodyLoaded(){
  ...
  $('.logo_path').css('stroke','none').css('fill','#fff');
  $('.time').text('1'.toHHMMSS());
  $('#cover_art img').attr('src', 'http://algraphixdecals.com/catalog/images/335d_1.JPG');
  $('#source .icon').attr('src', 'http://algraphixdecals.com/catalog/images/335d_1.JPG');
  $('#upnext .icon').attr('src', 'http://algraphixdecals.com/catalog/images/335d_1.JPG');
}
function init(){
  attachRdioEvents();
  if (R.authenticated()) {
